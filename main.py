...
def main():
    real_dir = r"C:\Users\walim\Github\AudioReplayAttackDetection\dataset\real\real_22050Hz"
    replay_dir = r"C:\Users\walim\Github\AudioReplayAttackDetection\dataset\replay\replay_22050Hz"

    X_real, y_real = create_dataset(real_dir, label=0)
    X_replay, y_replay = create_dataset(replay_dir, label=1)

    # Check if each class has at least two samples
    if len(X_real) < 2 or len(X_replay) < 2:
        print("Each class should have at least two samples for stratified splitting.")
        print("Combining both classes into one for training.")
        X = np.vstack((X_real, X_replay))
        y = np.hstack((y_real, y_replay))
    else:
        X = np.vstack((X_real, X_replay))
        y = np.hstack((y_real, y_replay))
